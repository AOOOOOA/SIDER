...
def rpy_to_carla_rotation_replay(roll, pitch, yaw):
    return carla.Rotation(
        roll=math.degrees(roll), pitch=math.degrees(pitch), yaw=math.degrees(yaw)
    )
...
def cyber_quaternion_to_carla_rotation_replay(cyber_quaternion):
    roll, pitch, yaw = quat2euler(
        [
            cyber_quaternion.qw,
            cyber_quaternion.qx,
    ...
    return rpy_to_carla_rotation_replay(roll, pitch, yaw)
...
# 只改变orientation 不改变 localization 的

def cyber_pose_to_carla_transform_orientation(cyber_pose):
    """
    由于数据读入方式的原因，这里我们保持position 不转换， 将Orientation 转换回carla 中的格式
    # """
    # print("cyber pose position", cyber_pose.position, type(cyber_pose.position))
    # print("cyber pose orientation", cyber_pose.orientation, type(cyber_pose.orientation))
    
    
    return carla.Transform(
        carla.Location(cyber_pose.position.x, cyber_pose.position.y, cyber_pose.position.z+0.03),
        cyber_quaternion_to_carla_rotation_replay(cyber_pose.orientation),
